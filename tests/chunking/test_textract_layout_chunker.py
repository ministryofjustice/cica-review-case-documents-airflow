...
def test_selects_correct_strategy_and_increments_index(document_metadata):
    ...
    with patch(
        "ingestion_pipeline.chunking.strategies.layout.textract_layout_chunker.ChunkMerger", autospec=True
    ) as mock_merger:
...
#     chunker = TextractLayoutDocumentChunker(strategy_handlers=strategy_handlers)
...
@patch("ingestion_pipeline.chunking.strategies.layout.textract_layout_chunker.logger")
