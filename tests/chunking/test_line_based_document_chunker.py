...
class TestLineBasedDocumentChunker:
    ...
    def test_chunk_empty_document_raises_chunk_error(self, chunker, sample_metadata):
        ...
        assert "Document cannot be None and must contain pages" in str(excinfo.value)

    def test_chunk_none_document_raises_chunk_error(self, chunker, sample_metadata):
        """Test that None document raises ChunkError wrapping ValueError."""
        with pytest.raises(ChunkError) as excinfo:
            chunker.chunk(None, sample_metadata)
        assert "Document cannot be None and must contain pages" in str(excinfo.value)

    def test_chunk_document_without_response_raises_error(self, chunker, sample_metadata):
        """Test that document without response raises ChunkException wrapped in ChunkError."""
        ...
        with pytest.raises(ChunkError) as excinfo:
            chunker.chunk(mock_doc, sample_metadata)
        assert "missing raw response" in str(excinfo.value)
...
class TestErrorHandling:
    ...
    def test_none_document_raises_error(self, sample_metadata):
        ...
        with pytest.raises(ChunkError, match="Document cannot be None"):
            chunker.chunk(cast(Any, None), sample_metadata)
