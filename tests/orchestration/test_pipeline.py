...
from src.chunking.schemas import DocumentBoundingBox, DocumentMetadata, OpenSearchDocument, ProcessedDocument
...
def mock_processed_data_with_chunks():
    ...
    # Create a mock ProcessedDocument (only chunks needed for these tests)
    mock_data = MagicMock(spec=ProcessedDocument)
    mock_data.chunks = chunks
    mock_data.pages = []  # Assuming pages are not relevant for this flow check
...
def test_orchestrator_initialization(mock_chunker, mock_indexer):
    ...
    orchestrator = IndexingOrchestrator(chunker=mock_chunker, chunk_indexer=mock_indexer)
...
def test_process_and_index_success(
    ...
    """
    # 1. Arrange: Set up the mock chunker to return data with chunks
...
def test_process_and_index_no_chunks_skips_indexing(
    ...
    """
    # 1. Arrange: Set up the mock chunker to return data with NO chunks
    ...
    # 3. Assert: Verify call sequence

    # Check that chunking was called (it must run)
...
def test_process_and_index_chunker_raises_exception(
    ...
    """
    # 1. Arrange: Set the chunker to raise an exception
    mock_chunker.chunk.side_effect = Exception("Simulated chunking failure")

    orchestrator = IndexingOrchestrator(chunker=mock_chunker, chunk_indexer=mock_indexer)

    # 2. Act & Assert: Run the pipeline and expect the exception
    with pytest.raises(Exception, match="Simulated chunking failure"):
        orchestrator.process_and_index(mock_textractor_doc, mock_document_metadata)

    # 3. Assert: Indexer should NOT have been called
    mock_indexer.index_documents.assert_not_called()
...
def test_process_and_index_indexer_raises_exception(
    ...
    """
    # 1. Arrange: Set up mock chunker to succeed and mock indexer to fail
    mock_chunker.chunk.return_value = mock_processed_data_with_chunks
    ...
    # 3. Assert: Chunker should still have been called
    mock_chunker.chunk.assert_called_once()
