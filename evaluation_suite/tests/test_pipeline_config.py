...
from evaluation_suite.search_evaluation import pipeline_config


def test_get_timestamp_format():
    """Test that get_timestamp returns a string in the expected format."""
    ts = pipeline_config.get_timestamp()
...
def test_get_date_folder_returns_today():
    ...
    folder = pipeline_config.get_date_folder()
...
def test_output_paths_are_correct():
    ...
    paths = pipeline_config.OUTPUT_PATHS
...
@patch("evaluation_suite.search_evaluation.pipeline_config.settings")
def test_get_active_search_types_exact_only(mock_settings):
    ...
    assert pipeline_config.get_active_search_types() == ["exact"]
...
def test_get_active_search_types_multiple(mock_settings):
    ...
    mock_settings.SEMANTIC_BOOST = 0
    types = pipeline_config.get_active_search_types()
...
def test_get_active_search_types_semantic_only(mock_settings):
    ...
    assert pipeline_config.get_active_search_types() == ["semantic_only"]
...
@patch("evaluation_suite.search_evaluation.pipeline_config.get_active_search_types")
def test_get_active_search_type_hybrid(mock_types):
    ...
    assert pipeline_config.get_active_search_type() == "hybrid"
...
def test_get_active_search_type_single(mock_types):
    ...
    assert pipeline_config.get_active_search_type() == "stemmed"


@patch("evaluation_suite.search_evaluation.pipeline_config.settings")
@patch("evaluation_suite.search_evaluation.pipeline_config.get_timestamp")
def test_get_search_config_returns_dict(mock_timestamp, mock_settings):
    ...
    mock_settings.FUZZY_BOOST = 0
    mock_settings.WILDCARD_BOOST = 0
    mock_settings.DATE_QUERY_BOOST = 4.0
    mock_settings.DATE_FORMAT_DETECTION = True
    mock_settings.CASE_FILTER = "26-711111"

    config = pipeline_config.get_search_config(num_chunks_indexed=243, chunking_strategy="textractor-word-stream")
    assert config["search_type"] in {"exact", "stemmed", "fuzzy", "wildcard", "semantic_only", "hybrid"}
    assert config["case_filter"] == "26-711111"
    ...
    assert config["date_format_detection"] is True
    assert config["chunking_strategy"] == "textractor-word-stream"
    assert config["num_chunks_indexed"] == 243
