...
def bootstrap_opensearch(index_if_empty: bool = True, case_ref: str | None = None) -> int | None:
    ...
    if chunk_count == 0:
        ...
        if chunk_count == 0:
            case_label = f" for case '{case_ref}'" if case_ref else ""
            logger.warning(
                "Ingestion completed but 0 chunks were indexed%s. "
                "Possible causes: (1) the source document is missing or empty; "
                "(2) the chunking strategy produced no valid blocks; "
                "(3) LocalStack lost its state after a Docker restart — if running locally, "
                "check that 'local-kta-documents-bucket' exists and contains the source PDF, "
                "then re-run ingestion. "
                "See local-dev-environment/init-scripts/01-create-aws-resources.sh for setup commands.",
                case_label,
            )
    else:
