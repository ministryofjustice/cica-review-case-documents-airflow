"""Search query builder for OpenSearch, mirroring frontend PR #209.
...
The query DSL mirrors the frontend (PR #209,
``api/DAL/utils/buildQueryJson/``) so evaluation searches reproduce what users
experience:
...
- For ``*-dates`` types, ``match_phrase`` clauses for each date format variant
  (boosted by ``date_boost``).
...
and a pre-scoping ``filter``) mirrors PR #209; only the transport differs. If a
...
def _build_hybrid_query(
    ...
    """Build a hybrid (keyword + vector [+ dates]) query mirroring PR #209.
    ...
    if include_dates:
        for date in extract_dates_for_search(query_text):
            should.append({"match_phrase": {"chunk_text": {"query": date, "boost": config.date_boost}}})
...
def create_hybrid_query(
    ...
    """Create a search query for the given search type, mirroring PR #209.
